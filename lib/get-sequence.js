...
function getSequence(site, callback) {
  ...
  function exportFastaCallback(error, response, body) {
    if (error) {
      ...
      var
        sequenceStart = site.position + command.positionOffset,
        sequenceEnd = sequenceStart + command.sequenceLength;

      // If searching on the reverse strand, count backwards rather than forwards from the position.
      if (site.direction === 'r') {
        sequenceEnd = site.position - command.positionOffset;
        sequenceStart = sequenceEnd - command.sequenceLength;
      }

      if (sequenceStart < 0) { sequenceStart = 0; }

      if (sequenceStart >= sequenceEnd) {
        callback(new Error('Cannot determine a sequence with a negative length. Check the position, position offset and sequence length options.'));
      } else {
        site.fasta = new Fasta(body);
        site.sequence = site.fasta.body.slice(sequenceStart, sequenceEnd);
